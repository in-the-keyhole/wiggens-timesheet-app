...
type Entry = { dayOfWeek: string, hours: number, notes?: string }
...
export default function Timesheets() {
  ...
  const [entries, setEntries] = useState<Entry[]>(DAYS.map(d => ({ dayOfWeek: d, hours: 0, notes: '' })))
  ...
  const submit = async () => {
    ...
    setEntries(DAYS.map(d => ({ dayOfWeek: d, hours: 0, notes: '' })))
    setStatus('OPEN')
  }
    ...
    <Stack gap={2}>
      ...
      {entries.map((en, idx) => (
        <Stack direction="row" gap={2} key={en.dayOfWeek} alignItems="center">
          ...
          <TextField label="Notes" value={en.notes ?? ''} sx={{ flex: 1 }}
            onChange={e => { const v = [...entries]; v[idx] = { ...en, notes: e.target.value }; setEntries(v) }} />
        </Stack>
      ))}
